...
const allStories = document.querySelector("#allStories");
...
$("body").on("submit", "#my-form", function (e) {
  ...
  let messagess = $("#messagess").val();
  if (firstname && lastname && userName && gender && headline && messagess) {
    let newData = {
      id: stories.length + 2,
      firstname: firstname,
      lastname: lastname,
    ...
    let output = `<li class="lies">
        <div class="card">
            <div class="card-body" style="width: 100%;">
                <div class="card_header">
                    <span class="motionless"> ${
                      newData.firstname.charAt(0) +
                      " " +
                      newData.lastname.charAt(0)
                    }</span>
                    <div class="card_user_name">
                        <div class="names">By- ${
                          newData.firstname + " " + newData.lastname
                        }</div>
                        <div class="genered">${newData.gender}</div>
                    </div>
                </div>
                <h2 class="mt-5">${newData.headline}</h2>
                <div id="content">
                    <p class="reducedStory">
                    
                    ${newData.message.slice(0, 400)}
                         -- <a id="show-more">Read-more</a><br>
                    </p>
                    <p class="extendedStory hide">
                ...
                <div class="liked">
                    <i id="like" class=" fa fa-thumbs-up" data-id="${
                      newData.id
                    }"></i>
                    <span class="like-number">${newData.like} </span>
                    <span class="like-numbe2">Likes</span>
                </div>
            </div>
        </div>
    </li>`;

    $("#allStories").prepend(output);
    $("#firstname").val("");
    $("#lastname").val("");
    $("#userName").val("");
    $("#gender").val("male");
...
const getAllStories = () => {
  ...
  stories.forEach((story) => {
    output += `<li class="lies">
        <div class="card">
            <div class="card-body" style="width: 100%;">
                <div class="card_header">
                    <span class="motionless"> ${
                      story.firstname.charAt(0) + " " + story.lastname.charAt(0)
                    }</span>
                    <div class="card_user_name">
                        <div class="names">By- ${
                          story.firstname + " " + story.lastname
                        }</div>
                        <div class="genered">${story.gender}</div>
                    </div>
                </div>
                <h2 class="mt-5">${story.headline}</h2>
                <div id="content">
                    <p class="reducedStory">
                ...
                <div class="liked">
                    ...
                    <span class="like-number">${story.like} </span>
                    <span class="like-numbe2">Likes</span>
                </div>
            </div>
        </div>
  ...
  allStories.innerHTML = output;
};

// const readMoreBtn = document.querySelector("#show-more");

$("body").on("click", "#show-more", function () {
  let parent = $(this).parent(".reducedStory");
  let siblingElement = $(this)
    .parent(".reducedStory")
    .siblings(".extendedStory");
  parent.addClass("hide");
  siblingElement.removeClass("hide");
...
$("body").on("click", "#like", function () {
  ...
  let nextSiblingItem = $(this).siblings(".like-number").text().split(" ")[0];
  console.log(nextSiblingItem);

  if ($(this).hasClass("likedd")) {
    $(this).removeClass("likedd");
    $(this)
      .siblings(".like-number")
      .text(Number(nextSiblingItem) - 1);
  } else {
    $(this).addClass("likedd");
    $(this)
      .siblings(".like-number")
      .text(Number(nextSiblingItem) + 1);
  }
